...
| 2026-02-13 | Change API group domain from `devopstoolkitseries.com` to `devopstoolkit.ai` | Rebranding — new domain is shorter and more aligned with the project's AI/ML focus | All KCL source files (`definition.k`, `compositions.k`), all test files, examples, and `CLAUDE.md` must replace `devopstoolkitseries.com` with `devopstoolkit.ai`; API version stays `v2` |
